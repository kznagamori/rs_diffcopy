...
- [2. 対象プラットフォーム・開発環境](#2-対象プラットフォーム開発環境)
- [3. 機能一覧（v1.0.0）](#3-機能一覧v100)
- [4. アプリケーション設定ファイル（settings.toml）](#4-アプリケーション設定ファイルsettingstoml)
- [5. CLI仕様](#5-cli仕様)
- [6. 差異判定](#6-差異判定)
- [7. ファイル状態別の扱い](#7-ファイル状態別の扱い)
- [8. 出力構造](#8-出力構造)
- [9. 進捗表示](#9-進捗表示)
- [10. サマリー出力形式](#10-サマリー出力形式)
- [11. 終了コード](#11-終了コード)
- [12. エラーハンドリング](#12-エラーハンドリング)
- [13. --force オプションの安全機能](#13---force-オプションの安全機能)
- [14. 文字エンコーディング](#14-文字エンコーディング)
- [15. 三者間差分機能](#15-三者間差分機能)
- [16. 将来の機能検討](#16-将来の機能検討)
...
# Windows コンソール出力設定
# ===================
# Windows版でコンソール出力時の文字エンコーディング
# コンソール出力の場合のみCP932（Shift-JIS）でエンコードして出力
# パイプでclipコマンド等にコピーする際の互換性のため
# ファイル出力（--summary, --excel等）はUTF-8のまま
# CP932で表現できない文字は「?」に置換される
# ※この設定はWindows版のみ有効。Linux/macOSでは常にUTF-8
...
- File Tree表示のためのパス解析には`std::path::Path::components()`を使用
- これによりWindows（`\`区切り）とLinux/macOS（`/`区切り）の両方で正しくTree構造が生成される
- パス区切り文字に依存しない実装により、全プラットフォームで同一の出力形式を保証
