...
| IT-109 | 除外パターンのパスコンポーネントマッチ | `__pycache__`等のネストしたパス | パスコンポーネントにマッチして除外される |
...
| ID | テスト名 | 説明 | 期待結果 |
|----|---------|------|----------|
| IT-401 | サマリーヘッダー | ヘッダー情報の確認 | Source/Target/Output/Date情報が含まれる |
...
| IT-405 | Optionsセクション | オプション指定時の表示 | 除外パターン、dry-run等が表示される |
| IT-406 | Added Filesセクション | 追加ファイルの詳細一覧 | ディレクトリとファイルが分類表示される |
| IT-407 | Modified Filesセクション | 変更ファイルの詳細一覧 | 変更ファイル一覧が表示される |
| IT-408 | Deleted Filesセクション | 削除ファイルの詳細一覧 | ディレクトリとファイルが分類表示される |
...
| ID | テスト名 | 説明 | 期待結果 |
|----|---------|------|----------|
| IT-801 | 個別パッチファイル生成 | --patchオプションで変更ファイルごとにパッチ生成 | .patchファイルが出力ディレクトリに作成される |
| IT-802 | 統合パッチファイル生成 | --patch-fileオプションで全変更を1ファイルに | 指定パスに統合パッチが作成される |
| IT-803 | 個別と統合の両方生成 | --patchと--patch-fileを同時指定 | 両方のパッチが生成される |
| IT-804 | バイナリファイルスキップ | バイナリファイルのパッチ生成 | スキップされ、サマリーに[skip]と記載 |
| IT-805 | サブディレクトリ内パッチ | ネストしたディレクトリのファイルパッチ | ディレクトリ構造を維持してパッチ作成 |
| IT-806 | パッチサマリーセクション | パッチ生成時のサマリー出力 | Patch Detailsセクションが表示される |
| IT-807 | パッチオプションサマリー | パッチオプションのサマリー表示 | Optionsセクションにパッチ設定が表示 |
| IT-808 | 設定ファイルでパッチ | patch=trueの設定ファイル | パッチファイルが生成される |
| IT-809 | git apply互換性 | 生成パッチをgit applyで適用 | 正常に適用される |

### 3.10 Excelレポートテスト

テストにはcalamineクレートを使用してExcelファイルの内容を検証します。

| ID | テスト名 | 説明 | 期待結果 |
|----|---------|------|----------|
| IT-901 | Excelファイル生成 | -Eオプションで差分ありの場合 | .xlsxファイルが作成される |
...
| IT-910 | Excelツリー階層検証 | ネストしたディレクトリ構造のツリー表示 | 階層が正しくセルに配置される |
| IT-911 | Excel全変更タイプ | 追加/変更/削除の全タイプ表示 | Detailsシートに全セクションが表示される |

### 3.11 出力フィルターテスト

| ID | テスト名 | 説明 | 期待結果 |
|----|---------|------|----------|
| IT-1001 | --filter-status基本 | 単一ステータス指定 | 指定ステータスのファイルのみ表示 |
| IT-1002 | --filter-status複数 | カンマ区切りで複数指定 | 指定した複数ステータスのファイルが表示 |
| IT-1003 | --filter-status all | allキーワード | 全ステータスが対象に追加される |
| IT-1004 | --filter-status ^除外 | ^プレフィックスで除外 | 指定ステータスが除外される |
| IT-1005 | --filter-status all,^unchanged | 除外組み合わせ | unchanged以外すべてが表示される |
| IT-1006 | --filter-status後勝ち | 同じステータスを複数指定 | 最後の指定が有効（後勝ち） |
| IT-1007 | --stats-only | 統計情報のみ | File Tree、詳細セクションが非表示 |
| IT-1008 | --no-tree | ツリー非表示 | File Treeセクションが非表示 |
| IT-1009 | --no-details | 詳細非表示 | 詳細セクションが非表示 |
| IT-1010 | フィルター統計表示 | フィルター時の統計 | "(filtered out)"が統計に表示される |
| IT-1011 | Excelフィルター適用 | Excelレポートへのフィルター適用 | フィルター後のファイルのみExcelに出力 |
| IT-1012 | 設定ファイルフィルター | filter_status設定 | 設定ファイルのフィルターが適用される |

### 3.12 三者間フィルターテスト

| ID | テスト名 | 説明 | 期待結果 |
|----|---------|------|----------|
| IT-1101 | 三者間--filter-status基本 | 単一ステータス指定 | 指定ステータスのみ表示 |
| IT-1102 | 三者間--filter-status conflict | conflictのみ | コンフリクトのみ表示 |
| IT-1103 | 三者間--filter-status all,^unchanged | 除外組み合わせ | unchanged以外すべて表示 |
| IT-1104 | 三者間Excelフィルター | Excelへのフィルター適用 | フィルター後のファイルのみExcelに出力 |
